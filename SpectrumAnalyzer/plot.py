...
class SpectrumPlotWidget:
    ...
    def create_plot(self):
        ...
        # remove option and expert
        self.plot.ctrlMenu = None
        self.plot.scene().contextMenu = None

        # add two actions
        view = self.plot.getViewBox()
        self.demodulation = view.menu.addAction('Demodulation')
        self.df = view.menu.addAction('DF')
    ...
    def recalculate_plot(self, data_storage):
        ...
        QTimer.singleShot(0, lambda: self.update_plot(data_storage, force=True))
        QTimer.singleShot(0, lambda: self.update_average(data_storage, force=True))
        QTimer.singleShot(0, lambda: self.update_peak_hold_max(data_storage, force=True))
        QTimer.singleShot(0, lambda: self.update_peak_hold_min(data_storage, force=True))
    ...
    def recalculate_persistence(self, data_storage):
        ...
        QTimer.singleShot(0, lambda: self.update_persistence(data_storage, force=True))
