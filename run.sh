...
rm -f Direction_Finder/_receiver/C/gate_control_fifo
mkfifo Direction_Finder/_receiver/C/gate_control_fifo

rm -f Direction_Finder/_receiver/C/sync_control_fifo
mkfifo Direction_Finder/_receiver/C/sync_control_fifo

rm -f Direction_Finder/_receiver/C/rec_control_fifo
mkfifo Direction_Finder/_receiver/C/rec_control_fifo

# Start programs at realtime priority levels
curr_user=$(whoami)
#sudo chrt -r 50 ionice -c 1 -n 0 ./_receiver/C/rtl_daq $BUFF_SIZE 2>/dev/null 1| sudo chrt -r 50 ./_receiver/C/sync $BUFF_SIZE 2>/dev/null 1| sudo chrt -r 50 ./_receiver/C/gate $BUFF_SIZE 2>/dev/null 1|sudo nice -n -20 sudo -u $curr_user python3 -O Window.py $BUFF_SIZE  &>/dev/null&

# Comment the above and uncomment the below to show all errors to the log files
sudo chrt -r 50 ionice -c 1 -n 0 ./Direction_Finder/_receiver/C/rtl_daq $BUFF_SIZE 2>log_rtl_daq 1| sudo chrt -r 50 ./Direction_Finder/_receiver/C/sync $BUFF_SIZE 2>log_sync 1| sudo chrt -r 50 ./Direction_Finder/_receiver/C/gate $BUFF_SIZE 2>log_gate 1| sudo nice -n -20 sudo -u $curr_user python3 -O  window.py $BUFF_SIZE $IPADDR &>log_python&
